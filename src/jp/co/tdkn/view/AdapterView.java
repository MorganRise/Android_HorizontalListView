 ...
 * {@link android.widget.Gallery Gallery} for commonly used subclasses of
 * AdapterView.
 * 
...
public abstract class AdapterView<T extends Adapter> extends ViewGroup {
    ...
    public long getItemIdAtPosition(int position) {
        T adapter = getAdapter();
        return (adapter == null || position < 0) ? INVALID_COL_ID : adapter
                .getItemId(position);
    }
    ...
    // return itemCount > 0
    // && (getFirstVisiblePosition() > 0 || getLastVisiblePosition() < itemCount
    // - 1);
    // }
    // return false;
    // }
    ...
    void checkSelectionChanged() {
        if ((mSelectedPosition != mOldSelectedPosition)
                || (mSelectedColId != mOldSelectedColId)) {
            selectionChanged();
